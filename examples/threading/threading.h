 ...
 */
struct thread_data {
    pthread_mutex_t *mutex;       // Pointer to the mutex to be used
    int wait_to_obtain_ms;        // Time to wait before obtaining the mutex (in milliseconds)
    int wait_to_release_ms;       // Time to hold the mutex before releasing it (in milliseconds)
    bool thread_complete_success; // Set to true if the thread completed successfully, false otherwise
};
